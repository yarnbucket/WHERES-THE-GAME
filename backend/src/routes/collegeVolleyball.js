...
const LEAGUES = [
  { league: "womens-college-volleyball", label: "NCAA Women's Volleyball", gender: "women" },
  { league: "mens-college-volleyball", label: "NCAA Men's Volleyball", gender: "men" }
];
...
async function fetchLeague(league, date) {
  ...
  const response = await fetch(url, {
    headers: { accept: "application/json", "user-agent": "WTG/0.1H8q" }
  });
  if (!response.ok) throw new Error(`ESPN College Volleyball ${league} request failed: ${response.status}`);
...
router.get("/", async (req, res, next) => {
  ...
  if (sportKey !== "collegevolleyball") return next();
  ...
  try {
    ...
    console.error("College Volleyball resolve error:", error);
    return res.status(500).json({ status: "error", sport: "collegevolleyball", date, count: 0, games: [] });
  }
});
